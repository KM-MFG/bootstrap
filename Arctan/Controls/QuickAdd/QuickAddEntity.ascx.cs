			...
			try
			{
				...
				using(SqlConnection cn = new SqlConnection(DB.GetDBConn()))
				{
					...
					using(SqlCommand cmd = new SqlCommand(insertSQL, cn))
					{
						...
						cmd.Parameters["@sename"].Value = SE.MungeName(entityName);
				...
				//reset cache
				AppLogic.SetAppConfig("CacheMenus", "False");
				AspDotNetStorefrontCore.AppLogic.m_RestartApp();
				AppLogic.SetAppConfig("CacheMenus", "True");
				//reset cache
