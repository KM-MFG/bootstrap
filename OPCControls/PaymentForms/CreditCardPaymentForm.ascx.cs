	...
	public override void SaveViewToModel()
	{
		...
		if(authorizeNet.IsCimEnabled)
		{
			...
			if(CBSaveToAccount.Checked && paymentProfileId < 1)
			{
				...
				{
					//Creating the CIM profile failed so skip that, but don't kill the checkout entirely
					AspDotNetStorefrontCore.SysLog.LogMessage("Authorize.NET CIM Profile creation failed", 
						errorMsg, 
						AspDotNetStorefrontCore.MessageTypeEnum.Informational, 
						AspDotNetStorefrontCore.MessageSeverityEnum.Error);
					ShowError(AspDotNetStorefrontCore.AppLogic.GetString("AspDotNetStorefrontGateways.AuthorizeNet.Cim.SubscriptionFailed", customer.LocaleSetting));
					CBSaveToAccount.Checked = CBSaveToAccount.Enabled = false;
				}
			}
