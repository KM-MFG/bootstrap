        ...
        System.Web.UI.UserControl
    {
        ...
        private void InitializePageState()
        {
            ...
            if (this.ShoppingCartModel.ShippingRequired)
            {
				...
				ShipAddressHeaderContent.Text = AppLogic.AppConfigBool("AllowShipToDifferentThanBillTo")
					? StringResourceProvider.GetString("smartcheckout.aspx.132")
					: StringResourceProvider.GetString("smartcheckout.aspx.169");
		...
		protected void PaymentModel_ProcessPaymentComplete(object source, ProcessPaymentCompleteEventArgs args)
		{
			...
			else if(args.Message == AspDotNetStorefrontCore.AppLogic.ro_PMPayPalExpressRedirect)
			{
				var faultRedirect = AspDotNetStorefrontGateways.Processors.PayPalController.GetECFaultRedirect(AdnsfCustomer);
				if(!String.IsNullOrEmpty(faultRedirect))
					Response.Redirect(faultRedirect);
			}
			else
			{
