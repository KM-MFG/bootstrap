...
<asp:Repeater ID="rptItems" runat="server" DataSource='<%# KitGroup.SelectableItems %>' OnItemDataBound="rptItems_ItemDataBound">
	<ItemTemplate>
		...
		<div class="kit-img-file">
			<asp:Image ID="imgFile" runat="server" Visible='<%# Container.DataItemAs<KitItemData>().HasCustomerUploadedImage %>' ImageUrl='<%# Container.DataItemAs<KitItemData>().CustomerUploadedImagePath %>' />
		</div>
	</ItemTemplate>
</asp:Repeater>
