        ...
        protected override void CreateChildControls()
        {
            ...
            if (DataSource != null && !DataSource.IsEmpty())
            {
                ...
                if (ShowShipping && !UseInAjaxMiniCart)
                {
                    ...
                    //shipping method
                    if (!DataSource.WithMultipleShippingAddresses && DataSource.FirstCartItem.ShippingMethod.Length != 0 || UseInAjaxMiniCart)
                    {
               ...
               { 
                    if (DataSource != null)
                    {
                        if (!this.DataSource.VatIsInclusive)
                        {
        ...
        private void AssignDataSourceContentToControls(ShoppingCart cart)
        {
            if (DataSource != null && !cart.IsEmpty())
            {
                ...
                else
                {
                    ...
                    {
                        if (!DataSource.WithMultipleShippingAddresses && DataSource.FirstCartItem.ShippingMethodID != 0 && DataSource.FirstCartItem.ShippingMethod.Length != 0)
                        {
