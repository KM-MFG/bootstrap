...
namespace AspDotNetStorefrontControls
{
	...
	public class BaseKitGroupControl : UserControl
	{
		...
		public string StockHint(KitItemData item)
		{
			...
			if(item.HasMappedVariant && !item.VariantHasStock && AppLogic.AppConfigBool("KitInventory.AllowSaleOfOutOfStock"))
			{
				if(AppLogic.AppConfigBool("KitInventory.ShowOutOfStockMessage"))
				{
					html = string.Format("<span class=\"out-stock-hint errorLg\">{0}</span>", AppLogic.GetString("OutofStock.DisplaySellableOutOfStockOnKitPage", ThisCustomer.SkinID, ThisCustomer.LocaleSetting));
				}
			}
