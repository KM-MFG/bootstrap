...
namespace AspDotNetStorefront
{
    ...
    public partial class checkoutshipping : SkinBase
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
			...
			if (!cart.IsAllDownloadComponents() && !cart.IsAllFreeShippingComponents() && !cart.IsAllSystemComponents() && cart.HasMultipleShippingAddresses() && cart.TotalQuantity() <= AppLogic.MultiShipMaxNumItemsAllowed() && cart.CartAllowsShippingMethodSelection && cart.TotalQuantity() > 1 && !checkoutByAmazon.IsCheckingOut)
            {
