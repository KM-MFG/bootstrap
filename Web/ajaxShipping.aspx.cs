...
namespace AspDotNetStorefront
{
	...
	public partial class ajaxShipping : System.Web.UI.Page
	{
		...
		public void WriteShippingXML(XmlWriter writer, int VariantID, decimal Price, decimal Weight, int CountryID, int StateID, int ZoneID)
		{
			...
			switch(ShipCalcID)
			{
						...
						using(SqlConnection con = new SqlConnection(DB.GetDBConn()))
						{
							...
							using(IDataReader rs = DB.GetRS(shipsql.ToString(), con))
							{
								while(rs.Read())
								{
									...
									{
										Decimal ThisShipCost = Shipping.GetShipByWeightCharge(ThisID, Weight);

										if(ThisShipCost != System.Decimal.Zero) //Don't add the fee to free methods.
						...
						using(SqlConnection con = new SqlConnection(DB.GetDBConn()))
						{
							...
							using(IDataReader rs = DB.GetRS(shipsql.ToString(), con))
							{
								while(rs.Read())
								{
									...
									{
										Decimal ThisShipCost = Shipping.GetShipByTotalCharge(ThisID, Price);
				...
				case Shipping.ShippingCalculationEnum.AllOrdersHaveFreeShipping:
					...
					writer.WriteElementString("Method1", String.Format("checkoutshipping.aspx.7", String.Empty));
					writer.WriteEndElement();
					break;
						...
						using(SqlConnection con = new SqlConnection(DB.GetDBConn()))
						{
							...
							using(IDataReader rs = DB.GetRS(shipsql.ToString(), con))
							{
								while(rs.Read())
								{
									...
									{
										Decimal ThisShipCost = Shipping.GetShipByFixedPercentageCharge(ThisID, Price);
						...
						using(SqlConnection con = new SqlConnection(DB.GetDBConn()))
						{
							...
							using(IDataReader rs = DB.GetRS(shipsql.ToString(), con))
							{
								while(rs.Read())
								{
									...
									{
										Decimal ThisShipCost = Shipping.GetVariantShippingCost(VariantID, ThisID);
						...
						using(SqlConnection con = new SqlConnection(DB.GetDBConn()))
						{
							...
							using(IDataReader rs = DB.GetRS(shipsql.ToString(), con))
							{
								while(rs.Read())
								{
									...
									{
										Decimal ThisShipCost = Shipping.GetShipByWeightAndZoneCharge(ThisID, Weight, ZoneID);
						...
						using(SqlConnection con = new SqlConnection(DB.GetDBConn()))
						{
							...
							using(IDataReader rs = DB.GetRS(shipsql.ToString(), con))
							{
								while(rs.Read())
								{
									...
									{
										Decimal ThisShipCost = Shipping.GetShipByTotalAndZoneCharge(ThisID, Price, ZoneID);
						...
						using(SqlConnection con = new SqlConnection(DB.GetDBConn()))
						{
							...
							using(IDataReader rs = DB.GetRS(shipsql.ToString(), con))
							{
								while(rs.Read())
								{
									...
									{
										Decimal ThisShipCost = Shipping.GetShipByTotalByPercentCharge(ThisID, Price);
		...
		protected ShippingMethods GetRates()
		{
			...
			return (ShippingMethods)realTimeShipping.GetRates(shipments, carriers, format, "ShippingMethod", "ShippingMethod", ShippingTaxRate, out RTShipRequest, out RTShipResponse, AppLogic.AppConfigNativeDecimal("ShippingHandlingExtraFee"), (decimal)MarkupPercent, realTimeShipping.ShipmentValue);
		}
	}
}
