...
namespace AspDotNetStorefront
{
	public class SkinBase : System.Web.UI.Page
	{
		...
		private readonly System.Collections.Generic.Dictionary<string, EntityHelper> m_EntityHelpers = new System.Collections.Generic.Dictionary<string, EntityHelper>();
		...
		private string GetTemplateName()
		{
			...
			if(!ThisCustomer.IsAdminUser && AppLogic.AppConfigBool("DisplayMaintenancePage.Enable"))
			{
				templateName = "maintenance.master";
				if(!CommonLogic.FileExists(SkinRoot + templateName))
				{
					templateName = "template.master";
				}
			}
				...
				if(m_IGD.Length != 0)
				{
					...
					if(PhoneCustomer != null && PhoneCustomer.HasCustomerRecord)
					{
						...
						if(PhoneCustomer.ThisCustomerSession.LastActivity >= DateTime.Now.AddMinutes(-ImpersonationTimeoutInMinutes))
						{
							...
							SkinID = AppLogic.GetStoreSkinID(AppLogic.StoreID());	//Force the admin user to the default skin (as the impersonated customer will be)
						}
						else
						{
							if(HttpContext.Current.Items.Contains("IsBeingImpersonated"))
		...
		public void SetMetaTags(String SETitle, String SEKeywords, String SEDescription)
		{
