            ...
            if (Page.IsValid)
            {
                ...
                if (ErrorMsg.Length == 0)
                {
                    try
                    {
                        ...
                        if (!Editing)
                        {
                            ...
                            // ok to add them:
                            Affiliate a = Affiliate.CreateAffiliate(CommonLogic.Left(EMailField, 100), p.SaltedPassword, null, false, CommonLogic.Left(FirstName.Text, 50), CommonLogic.Left(LastName.Text, 50), CommonLogic.Left(Name, 100), CommonLogic.Left(Company.Text, 50), Address1.Text.Replace("\x0D\x0A", ""), Address2.Text.Replace("\x0D\x0A", ""), Suite.Text, City.Text, State.Text, Zip.Text, Country.Text, Phone.Text, WebSiteName.Text, WebSiteDescription.Text, CommonLogic.Left(URL.Text, 80), 1, 0, 1, null, null, null, null, null, null, false, p.Salt);
                ...
                if (lblErrorMsg.Text.Length == 0)
                {
                    ...
                    try
                    {
                        ...
                        {
							if(!Request.Form.Keys[i].Contains("SearchBox") && !Request.Form.Keys[i].Contains("AffPassword") && !Request.Form.Keys[i].StartsWith("__"))
							{							
								var formFieldTitle = Request.Form.Keys[i];
								if(Request.Form.Keys[i].Contains("$"))
								{
									formFieldTitle = Request.Form.Keys[i].Substring(Request.Form.Keys[i].LastIndexOf('$') + 1);
								}								
								FormContents += string.Format("<b>{0}:</b> {1}<br />", formFieldTitle, Request.Form[Request.Form.Keys[i]] == string.Empty ? AppLogic.GetString("admin.nolinktext", SkinID, ThisCustomer.LocaleSetting) : Request.Form[Request.Form.Keys[i]]);
                            }
                        }
                        AppLogic.SendMail(string.Format("{0} New Member Notification", AppLogic.GetString("AppConfig.AffiliateProgramName", SkinID, ThisCustomer.LocaleSetting)), FormContents, true, AppLogic.AppConfig("MailMe_FromAddress"), AppLogic.AppConfig("MailMe_FromName"), AppLogic.AppConfig("AffiliateEMailAddress"), AppLogic.AppConfig("AffiliateEMailAddress"), AppLogic.AppConfig("MailMe_FromAddress"), AppLogic.MailServer());
                    }
                    catch { }

					lblSignupSuccess.Text = string.Format("CONGRATULATIONS AND WELCOME TO THE {0} PROGRAM!<br/><br/>Your sign-up was successful.<br/><br/><a href=\"lat_account.aspx\">Click here</a> to go to your {1} Account Page.", AppLogic.GetString("AppConfig.AffiliateProgramName", SkinID, ThisCustomer.LocaleSetting).ToUpperInvariant(), AppLogic.GetString("AppConfig.AffiliateProgramName", SkinID, ThisCustomer.LocaleSetting));
