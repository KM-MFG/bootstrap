...
namespace AspDotNetStorefrontAdmin
{
	...
	public partial class phoneorder : AdminPageBase
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				if(m_OrderNumber != 0)
				{
					...
					btnResetToMatchOriginal.Visible = true;
					btnEditClearCart.Visible = true;
					btnEditViewOldReceipt.Visible = true;
					helporderedit.Visible = true;
					backtoorderlink.NavigateUrl = AppLogic.AdminLinkUrl("order.aspx") + "?ordernumber=" + m_OrderNumber.ToString();
					...
					SetToImpersonationPageContext(ord.CustomerID, "../shoppingcart.aspx");
				}
			}
			else
			{
		...
		protected void btnCreateCustomerTop_Click(object sender, EventArgs e)
		{
			// create new customer
		...
		protected void btnSearch_Click(object sender, EventArgs e)
		{
			...
			String sql = string.Format("select top 50 CustomerID, FirstName, LastName, EMail from Customer  with (NOLOCK)  where deleted=0 and IsAdmin=0 and (convert(nvarchar(10),CustomerID)={0} or FirstName like {1} or LastName like {1} or CustomerID in (select CustomerID from Orders where convert(nvarchar(10),OrderNumber)={0}) or EMail like {1})", 
				DB.SQuote(txtSearch.Text.Trim()), 
				DB.SQuote("%" + txtSearch.Text.Trim() + "%"));
		...
		private void SetToImpersonationPageContext(int CustomerID, String PageName)
		{
		...
		protected void btnUseCustomer_Click(object sender, EventArgs e)
		{
			...
			if(EMailAlreadyTaken.Visible == false)
			{
				...
				SetToImpersonationPageContext(System.Int32.Parse(CustomerID.Text), "../default.aspx");
			}
		}
		...
		protected void btnCancelOrder_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				Response.Redirect(AppLogic.AdminLinkUrl("order.aspx") + "?ordernumber=" + m_OrderNumber.ToString());
			}
		}
		...
		protected void btnResetToMatchOriginal_Click(object sender, EventArgs e)
		{
			// Edit: Reset To Match Original Order
			if(m_OrderNumber != 0)
			{
				DB.ExecuteSQL("aspdnsf_EditOrder " + m_OrderNumber.ToString());
				SetToImpersonationPageContext(System.Int32.Parse(CustomerID.Text), "../shoppingcart.aspx");
			}
		}

		protected void btnEditClearCart_Click(object sender, EventArgs e)
		{
			// Edit: Clear Cart
			if(m_OrderNumber != 0)
			{
		...
		protected void btnEditViewOldReceipt_Click(object sender, EventArgs e)
		{
			// Edit: View Original Receipt
			if(m_OrderNumber != 0)
			{
				SetToImpersonationPageContext(System.Int32.Parse(CustomerID.Text), "../receipt.aspx?ordernumber=" + m_OrderNumber.ToString());
			}
		}

		protected void btnClearFailedTransaction_Click(object sender, EventArgs e)
		{
			DB.ExecuteSQL("delete from FailedTransaction where CustomerID=" + CustomerID.Text);
		}

		protected void btnReEditCustomer_Click(object sender, EventArgs e)
		{
			// re-edit customer
			CreateNewCustomerPanel.Visible = true;
			CustomerStatsPanel.Visible = false;
		...
		protected void btnRestartImpersonation_Click(object sender, EventArgs e)
		{
			// restart impersonation
			btnUseCustomer_Click(sender, e);
		}
