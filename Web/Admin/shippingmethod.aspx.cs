...
{
	public partial class ShippingMethod : AdminPageBase
	{
		...
		private void BindPage()
		{
			...
			{
				dbconn.Open();
				using(IDataReader rs = DB.GetRS("select ShippingMethodID, Name, DisplayName, ShipRushTemplate, IsRTShipping from ShippingMethod with (NOLOCK)  where ShippingMethodID=" + ShippingMethodID.ToString(), dbconn))
				{
					if(rs.Read())
					{
						Editing = true;
					...
					ShipRushTemplate.Text = DB.RSField(rs, "ShipRushTemplate");
		...
		SqlCommand CreateUpdateCommand()
		{
			var updateCommand = new SqlCommand(
				...
				BEGIN
					INSERT INTO ShippingMethod(ShippingMethodGUID, ShipRushTemplate, [Name], DisplayName) 
					VALUES (@GUID, @ShipRushTemplate, @Name, @DisplayName)
			...
			if(AppLogic.AppConfigBool("ShipRush.Enabled"))
				updateCommand.Parameters["@ShipRushTemplate"].Value = ShipRushTemplate.Text;
