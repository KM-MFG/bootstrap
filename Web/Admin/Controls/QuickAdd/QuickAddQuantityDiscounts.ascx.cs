	...
	public partial class QuickAddQuantityDiscounts : BaseControl
	{
		...
		private void CreateQuantityDiscountTable()
		{
			try
			{
				...
				{
					cn.Open();
					using(SqlCommand cmd = new SqlCommand(@"insert into quantitydiscount(QuantityDiscountGUID,Name,DiscountType) 
				...
				{
					conn.Open();
					using(IDataReader rs = DB.GetRS("select QuantityDiscountID, Name from QuantityDiscount with (NOLOCK) where QuantityDiscountGUID=" + DB.SQuote(newGuid.ToString()), conn))
					{
						rs.Read();
				...
				{
					cn.Open();
					using(SqlCommand cmd = new SqlCommand(@"insert into QuantityDiscountTable(QuantityDiscountTableGUID,QuantityDiscountID,LowQuantity,HighQuantity,DiscountPercent,CreatedOn) 
