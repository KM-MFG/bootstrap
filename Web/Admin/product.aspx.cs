...
{
	public partial class ProductEditor : AdminPageBase
	{
			...
			using(SqlConnection dbconn = new SqlConnection(DB.GetDBConn()))
			{
				...
				using(IDataReader rs = DB.GetRS("select * from Product   with (NOLOCK)  where ProductID=" + ProductId.ToString(), dbconn))
				{
					...
					if(Editing)
					{
						...
						//EXTENSION DATA
						txtExtensionData.Text = Server.HtmlDecode(DB.RSField(rs, "ExtensionData"));
						...
						if(EntitySpecs.m_HasIconPic)
						{
							...
							{
								Image1URL = AppLogic.LocateImageURL("Skins/skin_1/images/nopictureicon.gif", Locale);
							}
							if(Image1URL.Length != 0)
							{
								ltIcon.Text = "";
						...
						if(EntitySpecs.m_HasMediumPic)
						{
							...
							{
								Image2URL = AppLogic.LocateImageURL("Skins/skin_1/images/nopicture.gif", Locale);
							}
							if(Image2URL.Length != 0)
							{
								ltMedium.Text = "";
						...
						if(EntitySpecs.m_HasLargePic)
						{
							...
							{
								Image3URL = AppLogic.LocateImageURL("Skins/skin_1/images/nopicture.gif", Locale);
							}
							if(Image3URL.Length != 0)
							{
								ltLarge.Text = "";
			...
			var productParameters = new[]
				{
					...
					new SqlParameter("ExtensionData", txtExtensionData.Text),
