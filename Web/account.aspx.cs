...
namespace AspDotNetStorefront
{
	public partial class account : SkinBase
	{
		...
		private void SetAccountFields()
		{
			...
			ctrlAccount.DOBMonth = ThisCustomer.DateOfBirth.Month.ToString();
			ctrlAccount.DOBDay = ThisCustomer.DateOfBirth.Day.ToString();
			ctrlAccount.DOBYear = ThisCustomer.DateOfBirth.Year.ToString();
		...
		{
			RedirectToAddressEditor("Billing");
		}

		protected void AddShippingAddress_Click(object sender, EventArgs e)
		{
			RedirectToAddressEditor("Shipping");
		}

		void RedirectToAddressEditor(string addressType)
		{
			var returnUrl = String.Format("account.aspx?checkout={0}", Checkout);
			var redirectUrl = String.Format("address.aspx?add=true&addressType={0}&Checkout={1}&returnURL={2}", addressType, Checkout, Server.UrlEncode(returnUrl));
