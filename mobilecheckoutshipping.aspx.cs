...
namespace AspDotNetStorefront
{
    ...
    /// </summary>
    public partial class mobilecheckoutshipping : SkinBase
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!cart.IsAllDownloadComponents() && !cart.IsAllFreeShippingComponents() && !cart.IsAllSystemComponents() && cart.HasMultipleShippingAddresses() && cart.TotalQuantity() <= AppLogic.MultiShipMaxNumItemsAllowed() && cart.CartAllowsShippingMethodSelection && cart.TotalQuantity() > 1)
            {
        ...
        public void btnNewShipAddr_OnClick(object sender, System.EventArgs e)
        {
            ...
            if (IsValid)
            {
                ...
                if (ddlChooseShippingAddr.SelectedValue != "0")
                {
                    ...
                    pnlSelectShipping.Visible = true;
                ...
                else
                {
                    ...
                    pnlSelectShipping.Visible = false;
                }

            }
            else
            {
                ctrlShippingMethods.ErrorMessage += "" + AppLogic.GetString("checkoutshipping.aspx.23", SkinID, ThisCustomer.LocaleSetting) + "";
                foreach (IValidator aValidator in this.Validators)
                {
                    if (!aValidator.IsValid)
                    {
                        ctrlShippingMethods.ErrorMessage += "&bull; " + aValidator.ErrorMessage + "";
                    }
                }
                pnlSelectShipping.Visible = false;
