<%@ Page EnableViewState="true" Language="c#" Inherits="AspDotNetStorefrontAdmin.Orders" CodeFile="orders.aspx.cs" EnableEventValidation="false" MasterPageFile="~/App_Templates/Admin_Default/AdminMaster.master" ValidateRequest="false" %>
...
<asp:Content ContentPlaceHolderID="bodyContentPlaceholder" runat="server">
	<div class="admin-module">
		...
		<aspdnsf:FilteredListing runat="server"
			ID="FilteredListing"
			SqlQuery="select {0} OrderNumber, OrderDate, OrderTotal, Email, ShippingPhone, Orders.StoreId, Store.Name [StoreName], IsPrinted, TransactionState, ShippedOn, ShippingState FROM Orders left join Store on Orders.StoreID = Store.StoreID WHERE {1}"
			...
			<ActionBarTemplate>
				...
				<asp:HyperLink runat="server"
					...
					NavigateUrl="phoneorder.aspx"
					Text="<%$ Tokens: StringResource, admin.menu.PhoneOrder %>" />
			...
			<ExpandableFilters>
				...
				<aspdnsf:ListFilter ID="ListFilter1" runat="server"
					Label="<%$Tokens:StringResource, admin.order.PaymentMethod %>"
					FieldName="PaymentMethod">
					<Items>
						<asp:ListItem Value="CREDITCARD" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.CreditCard %>' />
						<asp:ListItem Value="GOOGLECHECKOUT" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.GoogleCheckout %>' />
						<asp:ListItem Value="CHECKOUTBYAMAZON" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.CheckoutByAmazon %>' />
						<asp:ListItem Value="PAYPAL" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.PayPal %>' />
						<asp:ListItem Value="PAYPALEXPRESS" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.PayPalExpress %>' />
						<asp:ListItem Value="PURCHASEORDER" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.PurchaseOrder %>' />
						<asp:ListItem Value="REQUESTQUOTE" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.RequestQuote %>' />
						<asp:ListItem Value="CHECKBYMAIL" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.Check %>' />
						<asp:ListItem Value="COD" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.COD %>' />
						<asp:ListItem Value="CODMONEYORDER" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.CODMoneyOrder %>' />
						<asp:ListItem Value="CODCOMPANYCHECK" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.CODCompanyCheck %>' />
						<asp:ListItem Value="CODNET30" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.CODNet30 %>' />
						<asp:ListItem Value="ECHECK" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.ECheck %>' />
						<asp:ListItem Value="MICROPAY" Text='<%$Tokens:StringResource, admin.orders.PaymentMethod.MicroPay %>' />
					</Items>
				</aspdnsf:ListFilter>
