...
{
	public partial class ShippingByWeightAndZone : AdminPageBase
	{
			...
			foreach (var column in ShippingGrid.Columns)
			{
				...
				if (indexCounter > 1 && column.GetType() == typeof(BoundField))
				{
					...
					if (field.DataField.Contains("MethodAmount_"))
					{
						...
						var insertSql = @"insert into ShippingWeightByZone(RowGUID, LowValue, HighValue, ShippingMethodID, ShippingCharge, ShippingZoneID) 
										values(@ShippingGuid, @LowAmount, @HighAmount, @ShippingMethodId, @ShippingCharge, @ZoneId)";
