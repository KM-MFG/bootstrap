...
{
	public partial class EntityEditor : AdminPageBase
	{
		...
		void LoadContent()
		{
			...
			using(var connection = new SqlConnection(DB.GetDBConn()))
			{
				...
				var entitySql = String.Format("select * from {0} with(nolock) where {0}ID = {1}", EntitySpecs.m_EntityName, EntityId);
					...
					if(EntityEditing)
					{
						...
						if(EntitySpecs.m_HasIconPic)
						{
							...
							if(!CommonLogic.FileExists(imageUrl))
								imageUrl = AppLogic.LocateImageURL("Skins/skin_1/images/nopictureicon.gif", ThisCustomer.LocaleSetting);
						...
						if(EntitySpecs.m_HasMediumPic)
						{
							...
							if(!CommonLogic.FileExists(imageUrl))
								imageUrl = AppLogic.LocateImageURL("Skins/skin_1/images/nopicture.gif", ThisCustomer.LocaleSetting);
		...
		void UpdateEntity()
		{
			...
			try
			{
				if(!EntityEditing)
				{
					...
					sql.AppendFormat("{0},", DB.SQuote(SE.MungeName(AppLogic.GetFormsDefaultLocale("Name", txtName.Text, locale, EntitySpecs, EntityId))));
					...
					Response.Redirect(String.Format("entity.aspx?entityid={0}&entityname={1}", EntityId, EntityName), false);
				}
				else
				{
					// ok to update:
					...
					sql.Append("SEName=" + DB.SQuote(SE.MungeName(AppLogic.GetFormsDefaultLocale("Name", txtName.Text, locale, EntitySpecs, EntityId))) + ",");
		...
		void HandleImage(string filename, string imageSize, HttpPostedFile imageFile)
		{
			...
			AppLogic.ResizeEntityOrObject(EntitySpecs.m_EntityName, tempImagePath, imagePath, imageSize, targetMimeType);

			if(imageSize == "large")
				AppLogic.CreateOthersFromLarge(EntitySpecs.m_EntityName, tempImagePath, filename, targetMimeType);

			AppLogic.DisposeOfTempImage(tempImagePath);
		}
