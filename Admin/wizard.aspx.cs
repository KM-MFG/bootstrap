    ...
    public partial class wizard : AdminPageBase
    {
        ...
        protected void loadData()
        {
			...
			if(AppLogic.TrustLevel == AspNetHostingPermissionLevel.Unrestricted)
            {
				...
				ConnectionStringsSection connectionStrings = (ConnectionStringsSection)webconfig.GetSection("connectionStrings");
        ...
        public void BuildPaymentMethodList(string payMethod)
        {
			...
			cbxCODMoneyOrder.Checked = paymentMethods.Contains("C.O.D. (Money Order)".ToLowerInvariant());
			cbxCODCompanyCheck.Checked = paymentMethods.Contains("C.O.D. (Company Check)".ToLowerInvariant());
			cbxCODNet30.Checked = paymentMethods.Contains("C.O.D. (Net 30)".ToLowerInvariant());
        ...
        public void BuildGatewayList()
        {
			...
			if(repGateways.DataSource == null)
            {
				...
				ds.Add(CreateGatewayData("PayPal Payflow Link", "PayFlowPro", "(also enables PayPal Express Checkout) - See <a href='http://www.aspdotnetstorefront.com/linkmanager.aspx?topic=9500manual&type=paypalpayflowlink' target='_blank'>Manual</a>."));
				ds.Add(CreateGatewayData("PayPal Payments Advanced", "PayFlowPro", "(also enables PayPal Express Checkout) - See <a href='http://www.aspdotnetstorefront.com/linkmanager.aspx?topic=9500manual&type=paypalpaymentsadvanced' target='_blank'>Manual</a>."));
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
			...
			var config = AppLogic.GetAppConfig(0, "UseSSL");
            ...
            {
				ctrlAlertMessage.PushAlertMessage(errors.ToString(), AlertMessage.AlertType.Error);
            }
            else
            {
        ...
        private string GetCheckedPaymentMethods()
        {
			...
			if(cbxCOD.Checked)
                temp += ", C.O.D.";
			if(cbxCODMoneyOrder.Checked)
				temp += ", C.O.D. (Money Order)";
			if(cbxCODCompanyCheck.Checked)
				temp += ", C.O.D. (Company Check)";
			if(cbxCODNet30.Checked)
				temp += ", C.O.D. (Net 30)";
