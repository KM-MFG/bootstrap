...
namespace AspDotNetStorefrontAdmin
{
	public partial class MailingTest : AdminPageBase
	{
		...
		{
			var emailAppConfig = AppLogic.GetAppConfig(storeId, name);
			if(emailAppConfig == null)
				emailAppConfig = AspDotNetStorefrontCore.AppConfig.Create(name, String.Empty, value, "EMAIL", true, storeId);
			else
				AspDotNetStorefrontCore.AppConfig.Update(emailAppConfig.AppConfigID, emailAppConfig.Description, value, emailAppConfig.GroupName, emailAppConfig.SuperOnly, storeId);
		}

		void SendTestReceiptEmail(Int32 storeId)
		{
			var xmlPackageName = AppLogic.AppConfig("XmlPackage.OrderReceipt");
			var xmlPackage = new XmlPackage2(xmlPackageName, null, SkinID, String.Empty, "ordernumber=999999");
			...
			AppLogic.SendMail(subject, body + AppLogic.AppConfig("MailFooter"), true, AppLogic.AppConfig("ReceiptEMailFrom", storeId, true), AppLogic.AppConfig("ReceiptEMailFromName", storeId, true), ThisCustomer.EMail, String.Empty, String.Empty, AppLogic.MailServer());
		}
		...
		void SendTestNewOrderNotification(Int32 storeId)
		{
			...
			if(SendToList.IndexOf(';') != -1)
				foreach(String s in SendToList.Split(';'))
					AppLogic.SendMail(subject, body + AppLogic.AppConfig("MailFooter", storeId, true), true, AppLogic.AppConfig("GotOrderEMailFrom", storeId, true), AppLogic.AppConfig("GotOrderEMailFromName", storeId, true), s.Trim(), s.Trim(), String.Empty, AppLogic.MailServer());
			else
				AppLogic.SendMail(subject, body + AppLogic.AppConfig("MailFooter", storeId, true), true, AppLogic.AppConfig("GotOrderEMailFrom", storeId, true), AppLogic.AppConfig("GotOrderEMailFromName", storeId, true), SendToList, SendToList, String.Empty, AppLogic.MailServer());
		}
