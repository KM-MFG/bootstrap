...
{
	public partial class TopicEditor : AdminPageBase
	{
		...
		protected void btnCopyToStore_Click(object sender, EventArgs e)
		{
			...
			Response.Redirect(String.Format("topic.aspx?topicId={0}", clonedTopicId), false);
			ApplicationInstance.CompleteRequest();
		}
		...
		bool SaveTopic()
		{
			...
			try
			{
				if(TopicId == null)
				{
					...
					Response.Redirect(String.Format("topic.aspx?topicId={0}", newTopicId), false);
					ApplicationInstance.CompleteRequest();
					return true;
				}
				else
		...
		public IEnumerable<int> UnassignedStoreIds(string topicName)
		{
			using(var connection = new SqlConnection(DB.GetDBConn()))
			{
				...
				using(IDataReader rs = DB.GetRS(
					sql: "select StoreId from Store where StoreId not in (select StoreId from Topic where Name = @topicName)",
