...
{
	public partial class ShippingByTotalAndZone : AdminPageBase
	{
			...
			foreach (var column in ShippingGrid.Columns)
			{
				...
				if (indexCounter > 1 && column.GetType() == typeof(BoundField))
				{
					...
					if (field.DataField.Contains("MethodAmount_"))
					{
						...
						var insertSql = @"insert into ShippingTotalByZone(RowGUID, LowValue, HighValue, ShippingMethodID, ShippingCharge, ShippingZoneID) 
