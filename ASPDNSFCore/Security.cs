...
namespace AspDotNetStorefrontCore
{
    ...
    public class Security
    {
		...
		/// <summary>
		/// Performs a security audit and returns a list of string (message)
		/// </summary>
		public static List<string> SecurityAudit(Customer currentCustomer, HttpRequest request)
		{
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.SSL", 1, currentCustomer.LocaleSetting), true));
			}
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.PathElement", 1, currentCustomer.LocaleSetting), true));
			}

			// 3. remove or change admin@aspdotnetstorefront.com. Cannot use the default credentials long-term.
            var defaultAdmin = new Customer("admin@aspdotnetstorefront.com");
			if(defaultAdmin.EMail == "admin@aspdotnetstorefront.com")
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.DefaultAdmin", 1, currentCustomer.LocaleSetting), true));
			}
			defaultAdmin = null;

			// 4. check MailMe_Server AppConfig Setting. Cannot Allow blank MailMe_Server AppConfig.
            var sMailServer = AppLogic.AppConfig("MailMe_Server");
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.MailServer", 1, currentCustomer.LocaleSetting), true));
			}

			// 5. check for version.aspx. Should be deleted.
			if(File.Exists(CommonLogic.SafeMapPath("~/version.aspx")))
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.VersionAspx", 1, currentCustomer.LocaleSetting), true));
			}

			// 6. check for admin\assetmanager folder. Should be deleted. 
			if(Directory.Exists(CommonLogic.SafeMapPath("assetmanager")))
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.AssetManager", 1, currentCustomer.LocaleSetting), true));
			}
			...
			{				
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.AdminDir", 1, currentCustomer.LocaleSetting), true));
			}
			...
			if(AppLogic.TrustLevel == AspNetHostingPermissionLevel.Unrestricted || AppLogic.TrustLevel == AspNetHostingPermissionLevel.High)
			{
				...
				{
					list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.CustomErrors", 1, currentCustomer.LocaleSetting), false));
				}
				customErrors = null;
			}
			...
			if(AppLogic.TrustLevel == AspNetHostingPermissionLevel.Unrestricted || AppLogic.TrustLevel == AspNetHostingPermissionLevel.High)
			{
                var CompileSection = (CompilationSection)WebConfigurationManager.OpenWebConfiguration("~").GetSection("system.web/compilation");
				if(CompileSection.Debug == true)
				{
					list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.Debug", 1, currentCustomer.LocaleSetting), false));
				}
			}
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.Encryption", 1, currentCustomer.LocaleSetting), false));
			}
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.WebConfigWritable", 1, currentCustomer.LocaleSetting), false));
			}
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.RootWritable", 1, currentCustomer.LocaleSetting), false));
			}

            var payPalEnabled = AppLogic.AppConfig("PaymentMethods").ContainsIgnoreCase("PayPal");

			// 13. PayPal is not enabled. 
			if(!payPalEnabled)
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.PaypalNotEnabled", 1, currentCustomer.LocaleSetting), false));
			}
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.BillMeLaterNotEnabled", 1, currentCustomer.LocaleSetting), false));
			}

			// 15. Checkout By Amazon is not enabled.
			if(!AppLogic.AppConfig("PaymentMethods").ContainsIgnoreCase("CheckoutByAmazon"))
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.CheckoutByAmazonNotEnabled", 1, currentCustomer.LocaleSetting), false));
			}

            var dotFeedInstalled = AppConfigManager.AppConfigExists("DotFeed.AccessKey");

			// 16. DotFeed is installed but not enabled. 
			if(dotFeedInstalled && AppLogic.AppConfig("DotFeed.AccessKey") == String.Empty)
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.DotFeedNotEnabled", 1, currentCustomer.LocaleSetting), false));
			}

			// 17. DotFeed is not installed.
			if(!dotFeedInstalled)
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.DotFeedNotInstalled", 1, currentCustomer.LocaleSetting), false));
			}
			...
			{
				list.Add(WriteAuditRow(AppLogic.GetString("admin.splash.aspx.security.MetaTagsNotSet", 1, currentCustomer.LocaleSetting), false));
			}
		...
		private static string WriteAuditRow(string message, bool iserror)
		{
			...
			if(iserror)
			{
				errorMessage = string.Format("<div class=\"row admin-row\"><div class=\"col-sm-1\"><i class=\"fa fa-times-circle\"></i></div><div class=\"col-sm-11\">{0}</div></div>", message);
			}
			else
			{
				errorMessage = string.Format("<div class=\"row admin-row\"><div class=\"col-sm-1\"><i class=\"fa fa-exclamation-triangle\"></i></div><div class=\"col-sm-11\">{0}</div></div>", message);
			}
			return errorMessage;
		}

		private static bool FileIsWriteable(string filename)
		{
		...
		private static bool FolderIsWriteable(string foldername)
		{
            var principal = WindowsIdentity.GetCurrent();
