...
namespace AspDotNetStorefrontCore
{
    ...
    public class DB
    {
		...
		public static void ExecuteSQL(String sql, SqlParameter[] parameters, SqlConnection connection)
		{
			if(CommonLogic.ApplicationBool("DumpSQL"))
				HttpContext.Current.Response.Write("SQL=" + sql + "\n");
		...
		static public int GetSqlN(String sql, SqlParameter[] parameters, SqlConnection connection)
		{
			int N = 0;

			using(var reader = DB.GetRS(sql, parameters, connection))
				if(reader.Read())
					N = DB.RSFieldInt(reader, "N");
