...
namespace AspDotNetStorefrontCore
{
	...
	/// or inserted into existing pages. Access /t-topicname.aspx to view topic as page.
	/// </summary>
	public class Topic
	{
		...
		void LoadFromDB(int StoreID)
		{
			...
			if(m_TopicID != 0)
			{
				String sql = String.Format("SELECT * from Topic with (NOLOCK) where Deleted=0 and Published=1 and TopicID={0} and (SkinID IS NULL or SkinID=0 or SkinID={1}) order by DisplayOrder, Name ASC", m_TopicID.ToString(), m_SkinID.ToString());
		...
		public bool Commit()
		{
			...
			if(ThisCustomer.IsAdminUser)
			{
				...
				if(!topicexists)
				{
					...
					sql.Append("insert into Topic(TopicGUID,Name,SkinID,DisplayOrder,Title,Description,Password,HTMLOk,RequiresDisclaimer,ShowInSiteMap,SEKeywords,SEDescription,SETitle) values(");
		...
		public static String GetTitle(int TopicID, String LocaleSetting)
		{
			...
			{
				con.Open();
				using(IDataReader rs = DB.GetRS(string.Format("select Title from Topic with (NOLOCK) where Deleted=0 and TopicID={0}", TopicID.ToString()), con))
				{
					if(rs.Read())
					{
		...
		{
			string tsql = "select Name,SkinID,DisplayOrder,Title,Description,Password,HTMLOk,RequiresDisclaimer,ShowInSiteMap,SEKeywords,SEDescription,SETitle,StoreID from Topic with (NOLOCK) where deleted=0 and topicid = " + this.TopicID;
			...
			using(SqlConnection conn = new SqlConnection(DB.GetDBConn()))
			{
				...
				using(IDataReader rs = DB.GetRS(tsql, conn))
				{
					if(rs.Read())
					{
						...
						sql.Append("insert into Topic(TopicGUID,Name,SkinID,DisplayOrder,Title,Description,Password,HTMLOk,RequiresDisclaimer,ShowInSiteMap,SEKeywords,SEDescription,SETitle,StoreID) values(");
