    ...
    public class XmlPackage2: IDisposable
    {
        ...
        private void InitializeSystemData()
        {
            ...
            using (StringReader sr = new StringReader("<System><IsAdminSite /><IsAdminSiteInt /><CustomerID /><DefaultVATSetting /><CustomerVATSetting /><UseVATSetting /><CustomerLevelID /><CustomerLevelName /><CustomerFirstName /><CustomerLastName /><CustomerFullName />" +
                "<CustomerRoles /><IsAdminUser /><IsSuperUser /><VAT.Enabled /><VAT.AllowCustomerToChooseSetting /><LocaleSetting /><CurrencySetting /><CurrencyDisplayLocaleFormat /><WebConfigLocaleSetting /><SqlServerLocaleSetting /><PrimaryCurrency />" +
            ...
            if (HttpContext.Current.Request.Cookies != null)
            {
                for (int i = 0; i <= HttpContext.Current.Request.Cookies.Count - 1; i++)
                {
                    ...
                    if (ParamName.Length != 0 
						&& !ParamName.Equals("asp.net_sessionid", StringComparison.InvariantCultureIgnoreCase) 
						&& !ParamName.Contains("aspxauth")
						&& !ParamName.Equals(".ASPXANONYMOUS", StringComparison.InvariantCultureIgnoreCase))
                    {
                        try
                        {
