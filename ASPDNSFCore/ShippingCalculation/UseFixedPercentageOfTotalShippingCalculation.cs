...
{
    /// <summary>
    /// Summary for UseFixedPercentageOfTotal
    /// </summary>
    public class UseFixedPercentageOfTotalShippingCalculation : ShippingCalculation
    {
        /// <summary>
        /// Represent calculation for UseFixedPercentageOfTotal
        /// </summary>
        /// <returns>return collection of shipping method based on the computation of UseFixedPercentageOfTotal</returns>
            ...
            using (SqlConnection dbconn = new SqlConnection(DB.GetDBConn()))
            {
                ...
                using (IDataReader reader = DB.GetRS(shipsql.ToString(), dbconn))
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            decimal freight = Shipping.GetShipByFixedPercentageCharge(thisMethod.Id, SubTotalWithoutDownload); // exclude download items!
