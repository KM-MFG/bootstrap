...
";
        ...
        public virtual String AjaxShippingEstimator(String sVariantID)
        {
			...
			tmpS.Append("</label>");
			tmpS.Append("<input type=\"text\" class=\"form-control AjaxShippingZipInput\" type=\"text\" maxlength=\"16\" id=\"PostalCode\" name=\"PostalCode\" onkeyup=\"javascript:getShipping();\"/>");
            tmpS.Append("</div>");

            tmpS.Append("<div id=\"AjaxShippingEstimate\">");
			tmpS.Append("	<h5 class=\"AjaxShippingLabel\">Shipping Estimate:</h5>");
			tmpS.Append("	<div id=\"AjaxShippingLoader\"><span>Loading...</span></div>");
			tmpS.Append("	<div id=\"AjaxShippingError\" class=\"error-wrap\"></div>");
			tmpS.Append("	<div id=\"AjaxShippingQuote\"></div>");
            tmpS.Append("</div>");
		...
		public virtual string TopicLink(String sTopicName, String sIncludeATag)
		{
			...
			bool IncludeATag = IV.ValidateBool("IncludeATag", sIncludeATag);
			string result = String.Empty;
			result = SE.MakeDriverLink(TopicName);
			if (IncludeATag)
			{
				result = "<a href=\"" + result + "\">";
			}
        ...
        public virtual string AddtoCartFormERP(string sProductID, string sVariantID, String sColorChangeProductImage, String sVariantStyleFlag)
        {
            ...
            if (ProductID != 0 &&
                VariantID != 0)
            {
                result = ShoppingCart.GetAddToCartForm(ThisCustomer, false, AppLogic.AppConfigBool("ShowWishButtons"), ProductID, VariantID, ThisCustomer.SkinID, 0, ThisCustomer.LocaleSetting, ColorChangeProductImage, VariantStyle);
            }
            else
            {
                result = String.Empty;
        ...
        public virtual string AddtoCartForm(string sProductID, string sVariantID, String sColorChangeProductImage)
        {
            ...
            if (ProductID != 0 &&
                VariantID != 0)
            {
                result = ShoppingCart.GetAddToCartForm(ThisCustomer, false, AppLogic.AppConfigBool("ShowWishButtons"), ProductID, VariantID, ThisCustomer.SkinID, 0, ThisCustomer.LocaleSetting, ColorChangeProductImage, VariantStyleEnum.RegularVariantsWithAttributes);
            }
            else
            {
                result = String.Empty;
        ...
        public virtual string AddtoCartForm(string sProductID, string sVariantID, String sColorChangeProductImage, String sShowWishListButton)
        {
            ...
            if (ProductID != 0 &&
                VariantID != 0)
            {
                result = ShoppingCart.GetAddToCartForm(ThisCustomer, false, ShowWishListButton, ProductID, VariantID, ThisCustomer.SkinID, 0, ThisCustomer.LocaleSetting, ColorChangeProductImage, VariantStyleEnum.RegularVariantsWithAttributes);
            }
            else
            {
                result = String.Empty;
        ...
        public virtual string GetKitPrice(String sProductID, string sPrice, string sSalePrice, string sExtendedPrice, String sHidePriceUntilCart, string sColors, String sShoppingCartRecID, String intTaxClassID)
        {
            ...
            if (KitIsComplete)
            {
                ...
                {
                    results.Append(ShoppingCart.GetAddToCartForm(ThisCustomer, false, true, AppLogic.AppConfigBool("ShowWishButtons"), ProductID, AppLogic.GetDefaultProductVariant(ProductID), ThisCustomer.SkinID, 1, ThisCustomer.LocaleSetting, !pig.IsEmpty(), VariantStyleEnum.RegularVariantsWithAttributes));
                }
                results.Append("</p>");
            }
            else
