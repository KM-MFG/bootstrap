...
namespace AspDotNetStorefrontCore
{
    ...
    public static class ExtensionMethods
    {
        ...
        public static List<string> AddCommaDelimited(this List<string> destination, string commaDelimited)
        {
		...
		public static TListControl AddItems<TListControl>(this TListControl listControl, IEnumerable<string> items)
			where TListControl : ListControl
		{
			if(listControl == null)
				return listControl;
		...
		public static TListControl SelectAllByValue<TListControl>(this TListControl listControl, IEnumerable<string> values, IEqualityComparer<string> equalityComparer = null)
			where TListControl : ListControl
		{
			equalityComparer = equalityComparer ?? EqualityComparer<string>.Default;
			values = values ?? Enumerable.Empty<string>();
			return SelectAll(listControl, item => values.Contains(item.Value, equalityComparer));
		}

		public static TListControl SelectAllByText<TListControl>(this TListControl listControl, IEnumerable<string> texts, IEqualityComparer<string> equalityComparer = null)
			where TListControl : ListControl
		{
			equalityComparer = equalityComparer ?? EqualityComparer<string>.Default;
			texts = texts ?? Enumerable.Empty<string>();
			return SelectAll(listControl, item => texts.Contains(item.Text, equalityComparer));
		}

		public static TListControl SelectAll<TListControl>(this TListControl listControl, Func<ListItem, bool> selectionPredicate)
			where TListControl : ListControl
		{
			if(listControl == null)
				return listControl;

			// Find zero or more matching selected items. Ensure all other items are unselected.
			foreach(ListItem item in listControl.Items)
				item.Selected = selectionPredicate(item);
