...
namespace AspDotNetStorefrontCore
{
	...
	public class SkinProvider : ISkinProvider
	{
		...
		{
			var skins = Directory
				.GetDirectories(HostingEnvironment.MapPath("~/App_Templates/"), "Skin_*")
				.Select(skinPath => Path.GetFileName(skinPath))		// It says GetFileName(), but it's correct (GetDirectoryName() always gets the name of the parent node, which is App_Templates)
				.Select(skinName => GetSkinByName(skinName))
