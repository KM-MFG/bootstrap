    ...
    public class Customer : IIdentity
    {
		...
		static public int CreateCustomerRecord(string email, string password, object skinId, object affiliateId, string referrer, object isAdmin, string lastIpAddress, string localeSetting, object over13Checked, string currencySetting, object vatSetting, string vatRegistrationId, object customerLevelId)
		{
			...
			using(var command = new SqlCommand())
			{
				...
				command.Parameters.Add(new SqlParameter("@Password", SqlDbType.NVarChar, 200));
				command.Parameters.Add(new SqlParameter("@SkinID", SqlDbType.Int, 4));
				command.Parameters.Add(new SqlParameter("@AffiliateID", SqlDbType.Int, 4));
				command.Parameters.Add(new SqlParameter("@Referrer", SqlDbType.NText));
				command.Parameters.Add(new SqlParameter("@IsAdmin", SqlDbType.TinyInt, 1));
				command.Parameters.Add(new SqlParameter("@LastIPAddress", SqlDbType.VarChar, 40));
				command.Parameters.Add(new SqlParameter("@LocaleSetting", SqlDbType.NVarChar, 20));
				command.Parameters.Add(new SqlParameter("@Over13Checked", SqlDbType.TinyInt, 1));
				command.Parameters.Add(new SqlParameter("@CurrencySetting", SqlDbType.NVarChar, 20));
				command.Parameters.Add(new SqlParameter("@VATSetting", SqlDbType.Int, 4));
				command.Parameters.Add(new SqlParameter("@VATRegistrationID", SqlDbType.NVarChar, 100));
				command.Parameters.Add(new SqlParameter("@CustomerLevelID", SqlDbType.Int, 4));
				command.Parameters.Add(new SqlParameter("@CustomerID", SqlDbType.Int, 4)).Direction = ParameterDirection.Output;

				command.Parameters["@Email"].Value = (object)email ?? DBNull.Value;
				command.Parameters["@Password"].Value = (object)password ?? DBNull.Value;
				command.Parameters["@SkinID"].Value = skinId ?? DBNull.Value;
				command.Parameters["@AffiliateID"].Value = affiliateId ?? DBNull.Value;
				command.Parameters["@CustomerLevelID"].Value = customerLevelId ?? 0;		//JH soft default customer level should apply when 0
				command.Parameters["@Referrer"].Value = (object)referrer ?? DBNull.Value;
				command.Parameters["@IsAdmin"].Value = isAdmin ?? DBNull.Value;
				command.Parameters["@LastIPAddress"].Value = (object)lastIpAddress ?? DBNull.Value;
				command.Parameters["@LocaleSetting"].Value = (object)localeSetting ?? DBNull.Value;
				command.Parameters["@Over13Checked"].Value = over13Checked ?? DBNull.Value;
				command.Parameters["@CurrencySetting"].Value = (object)currencySetting ?? DBNull.Value;
				command.Parameters["@VATSetting"].Value = vatSetting ?? DBNull.Value;
				command.Parameters["@VATRegistrationID"].Value = vatRegistrationId ?? String.Empty;

				try
				{
					connection.Open();
					command.ExecuteNonQuery();
					customerId = Int32.Parse(command.Parameters["@CustomerID"].Value.ToString());
				}
				catch { }
        ...
        /// <returns></returns>
        public static string UpdateCustomerStatic(int CustomerID, object CustomerLevelID, string Email, string SaltedAndHashedPassword, object SaltKey, string DateOfBirth, string Gender, string FirstName, string LastName, string Notes, object SkinID, string Phone, object AffiliateID, string Referrer, string CouponCode, object OkToEmail, object IsAdmin, object BillingEqualsShipping, string LastIPAddress, string OrderNotes, string RTShipRequest, string RTShipResponse, string OrderOptions, string LocaleSetting, object MicroPayBalance, object RecurringShippingMethodID, string RecurringShippingMethod, object BillingAddressID, object ShippingAddressID, object CODCompanyCheckAllowed, object CODNet30Allowed, string ExtensionData, string FinalizationData, object Deleted, object Over13Checked, string CurrencySetting, object VATSetting, String VATRegistrationID, object StoreCCInDB, object IsRegistered, object LockedUntil, object AdminCanViewCC, object BadLogin, object Active, object PwdChangeRequired, object RegisterDate)
        {
		...
		public string UpdateCustomer(
			...
			bool? codCompanyCheckAllowed = null,
			bool? codNet30Allowed = null,
    ...
    public class GridCustomer
    {
        ...
        public GridCustomer(int CustomerID)
        {
            using (SqlConnection conn = new SqlConnection(DB.GetDBConn()))
            {
                ...
                using (IDataReader rs = DB.GetRS("select CustomerID, FirstName, LastName, IsAdmin, CustomerLevelID, Email, OkToEmail, CreatedOn, IsRegistered, LockedUntil, Active, Deleted from dbo.Customer with(NOLOCK) where CustomerID=" + CustomerID.ToString(), conn))
                {
                    if (rs.Read())
                    {
                        m_customerid = DB.RSFieldInt(rs, "CustomerID");
		...
		#region Public Methods
		/// <summary>
		/// Soft Deletes (sets Deleted=1) in the database
		/// </summary>
		...
		public bool Delete()
		{
			...
			{
				m_deleted = true;
				AppLogic.eventHandler("DeleteCustomer").CallEvent("&DeleteCustomer=true&DeletedCustomerID=" + m_customerid.ToString());
				DB.ExecuteSQL("update Customer set deleted=1, EMail=" + DB.SQuote("deleted_") + "+EMail where CustomerID=" + m_customerid.ToString());
			}
			catch (Exception ex)
			{
		...
		/// <summary>
		/// UnDeletes a soft deleted (set Deleted=0) in the database
		/// </summary>
		/// <returns>Boolean value indicating success</returns>
		public bool UnDelete()
		{
			...
			{
				m_deleted = false;
				if (m_email.StartsWithIgnoreCase("deleted_"))
				{
					string emailUndeleted = m_email.Remove(0, "deleted_".Length);
					DB.ExecuteSQL(string.Format("update Customer set deleted=0, EMail={0} where CustomerID={1}", DB.SQuote(emailUndeleted), m_customerid.ToString()));
				}
				else
				{
					throw new Exception(m_customerid + " email " + m_email + " could not be undeleted. Email does not start with \"deleted_\" ");
				}
