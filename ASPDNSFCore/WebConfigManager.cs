        ...
        public List<Exception> Commit()
        {
            ...
            try
            {
				...
				if(appSection.SectionInformation.IsProtected || connectionStrings.SectionInformation.IsProtected)
                {
					...
					LogSecurityEvent("Web.config file decrypted", "The web.config appSettings and connectionString sections have been automatically decrypted to support editing.  They will be re-encrypted after editing if ProtectWebConfig is true.", spa);
                }
				...
				if(m_protectWebConfig && !connectionStrings.SectionInformation.IsProtected)
				{
					...
					LogSecurityEvent("Web.config file encrypted", "The web.config connectionStrings section has been encrypted.", spa);
				}
				...
				{
					connectionStrings.SectionInformation.UnprotectSection();
					LogSecurityEvent("Web.config file decrypted", "The web.config connectionStrings section has been decrypted.", spa);
