    ...
    public partial class ShoppingCart
    {
        ...
        public ShoppingCart(SqlTransaction DBTrans, int SkinID, Customer ThisCustomer, CartTypeEnum CartType, int OriginalRecurringOrderNumber, bool OnlyLoadRecurringItemsThatAreDue, bool IsAjaxMiniCart)
        {
                ...
                CommonLogic.GetThisPageName(false).Equals("shoppingcart.aspx", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                {
                    this.AddItem(m_ThisCustomer, 0, AppLogic.MicropayProductID, AppLogic.MicropayVariantID, 0, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, CartTypeEnum.ShoppingCart, true, false, System.Decimal.Zero);
                }
            ...
            if (ProductRequires.Length != 0 && m_CartType != CartTypeEnum.RecurringCart) // Don't add required products if this is a recurring cart
            {
                ...
                foreach (DictionaryEntry de in ht)
                {
                    try
                    {
                        ...
                        {
                            AddItem(ThisCustomer, ThisCustomer.PrimaryShippingAddressID, PID, VID, deltaN, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, CartType, false, true, System.Decimal.Zero);
                        }
                    }
                    catch { }
                }
            ...
            if (CustomerID != 0)
            {
                using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                {
                    ...
                    string query = string.Format(
                                    ...
                                    WHERE sc.CustomerID = {1} AND sc.CartType = {2} AND ((p.Published = 1 AND pv.Published = 1 AND p.Deleted = 0 AND pv.Deleted = 0)) and ({3} = 0 or sc.StoreID = {4})", CommonLogic.IIF(AppLogic.GlobalConfigBool("AllowProductFiltering") == true, 1, 0), CustomerID, ((int)CartType), CommonLogic.IIF(AppLogic.GlobalConfigBool("AllowShoppingcartFiltering") == true, 1, 0), AppLogic.StoreID());
        ...
        private void LoadFromDB(CartTypeEnum CartType)
        {
            ...
            if (CartType == CartTypeEnum.ShoppingCart && m_ThisCustomer.CustomerID != 0)
            {
                ...
                if (ThisCustomer.PrimaryShippingAddressID != 0)
                {
                    ...
                    // later in the cart where multi-ship items, etc, are involved!
                    DB.ExecuteSQL("update shoppingcart set ShippingAddressID=" + ThisCustomer.PrimaryShippingAddressID.ToString() + " where CartType=" + ((int)CartTypeEnum.ShoppingCart).ToString() + " and CustomerID=" + ThisCustomer.CustomerID.ToString() + " and (ShippingAddressID=0 or ShippingAddressID not in (select AddressID from Address  with (NOLOCK)  where CustomerID=" + m_ThisCustomer.CustomerID.ToString() + "))", m_DBTrans);
                }
            }
        ...
        public void SetCoupon(String newCoupon, bool UpdateCartObject)
        {
            ...
            else
            {
                ...
                if (UpdateCartObject)
                {
                    if (this.Coupon.CouponType == CouponTypeEnum.GiftCard || !(GC.SerialNumber.Length == 0))
                    {
                        ClearCoupon();
                        SqlConnection con = null;
                        IDataReader rs = null;
        ...
        public String DisplayMultiShipMethodSelector(bool VarReadOnly, Customer ViewingCustomer)
        {
            ...
            {
                return tmpS.ToString(); // can only do this action on the shoppingcart proper, not wish
            }
            if (IsEmpty())
            {
        ...
        public ShippingMethodCollection GetShippingMethods(Address usingThisAddress, decimal taxRate, bool returnInStorePickupRate)
        {
            ...
            if (shouldWeGetShippingMethods)
            {
                ...
                if (shippingCalculation != null)
                {
                    ...
                    shippingCalculation.HandlingExtraFee = AppLogic.AppConfigUSDecimal("ShippingHandlingExtraFee");
        ...
        private String GetLineItemDescription(CartItem c, bool ShowLinkBack, bool VarReadOnly, bool ShowMultiShipAddressUnderItemDescription)
        {
            ...
            if (IsAKit)
            {
                if (AppLogic.AppConfigBool("ShowEditButtonInCartForKitProducts"))
                {
                    bool isKit2 = false;
            ...
            if (this.HasMultipleShippingAddresses() && ShowMultiShipAddressUnderItemDescription && !c.IsDownload && !c.IsSystem)
            {
        ...
        /// <returns></returns>
        public int AddItem(Customer ThisCustomer, int ShippingAddressID, int ProductID, int VariantID, int Quantity, String ChosenColor, String ChosenColorSKUModifier, String ChosenSize, String ChosenSizeSKUModifier, String TextOption, CartTypeEnum CartType, bool UpdateCartObject, bool IsRequired, decimal CustomerEnteredPrice)
        {
            return AddItem(ThisCustomer, ShippingAddressID, ProductID, VariantID, Quantity, ChosenColor, ChosenColorSKUModifier, ChosenSize, ChosenSizeSKUModifier, TextOption, CartType, UpdateCartObject, IsRequired, CustomerEnteredPrice, null);
        }

        public int AddItem(Customer ThisCustomer, int ShippingAddressID, int ProductID, int VariantID, int Quantity, String ChosenColor, String ChosenColorSKUModifier, String ChosenSize, String ChosenSizeSKUModifier, String TextOption, CartTypeEnum CartType, bool UpdateCartObject, bool IsRequired, decimal CustomerEnteredPrice, KitComposition preferredComposition)
        {
            return AddItem(ThisCustomer, ShippingAddressID, ProductID, VariantID, Quantity, ChosenColor, ChosenColorSKUModifier, ChosenSize, ChosenSizeSKUModifier, TextOption, CartType, UpdateCartObject, IsRequired, CustomerEnteredPrice, preferredComposition, false);
        }
        ...
        /// <returns></returns>
        public int AddItem(Customer ThisCustomer, int ShippingAddressID, int ProductID, int VariantID, int Quantity, String ChosenColor, String ChosenColorSKUModifier, String ChosenSize, String ChosenSizeSKUModifier, String TextOption, CartTypeEnum CartType, bool UpdateCartObject, bool IsRequired, decimal CustomerEnteredPrice, KitComposition preferredComposition, bool isAGift)
        {
            if (null != preferredComposition)
            {
                foreach (CartItem c in m_CartItems)
                {
                    if (ShippingAddressID == c.ShippingAddressID && c.ProductID == ProductID && c.VariantID == VariantID && c.ChosenSize == ChosenSize && c.TextOption == TextOption && c.ChosenColor == ChosenColor && (CustomerEnteredPrice == System.Decimal.Zero || CustomerEnteredPrice == c.Price))
                    {
        ...
        public void RemoveItem(int cartRecordID, bool Recurse)
        {
            ...
            if (cartRecordID != 0)
            {
                ...
                try
                {
					...
					using(SqlConnection con2 = new SqlConnection(DB.GetDBConn()))
					{
						con2.Open();
        ...
        // NOT updated for multi-ship (yet)
        ...
        /// <returns></returns>
        static public String GetAddToCartForm(Customer ThisCustomer, bool forPack, bool showWishListButton, int ProductID, int VariantID, int SkinID, int DisplayFormat, String LocaleSetting, bool ColorChangeProductImage, VariantStyleEnum VariantStyle)
        {
            return GetAddToCartForm(ThisCustomer, forPack, false, showWishListButton, ProductID, VariantID, SkinID, DisplayFormat, LocaleSetting, ColorChangeProductImage, VariantStyle);
        }
        ...
        /// <param name="VariantStyle">The variant style.</param>
        /// <returns></returns>
        static public String GetAddToCartForm(Customer ThisCustomer, bool forPack, bool forKit, bool showWishListButton, int ProductID, int VariantID, int SkinID, int DisplayFormat, String LocaleSetting, bool ColorChangeProductImage, VariantStyleEnum VariantStyle)
        {
            return GetAddToCartForm(ThisCustomer, forPack, forKit, showWishListButton, ProductID, VariantID, SkinID, DisplayFormat, LocaleSetting, ColorChangeProductImage, VariantStyle, false);
        }
        ...
        static public String GetAddToCartForm(Customer ThisCustomer, bool forPack, bool forKit, bool showWishListButton, int ProductID, int VariantID, int SkinID, int DisplayFormat, String LocaleSetting, bool ColorChangeProductImage, VariantStyleEnum VariantStyle, bool isKit2)
        {
            ...
            else
            {
                ...
                tmpS.AppendFormat(" <input type=\"button\" id=\"AddToCartButton_{0}_{1}\" name=\"AddToCartButton_{0}_{1}\" class=\"button call-to-action add-to-cart\" value=\"{2}\">", ProductID, VariantID, AppLogic.GetString("AppConfig.CartButtonPrompt", SkinID, LocaleSetting));
            }
            ...
            if (AppLogic.AppConfigBool("ShowWishButtons") && showWishListButton)
            {
                ...
                if (AppLogic.AppConfigBool("AddToCart.UseImageButton") && AppLogic.AppConfig("AddToCart.AddToWishButton") != "")
                {
                    ...
                    tmpS.AppendFormat("<input type=\"image\" id=\"AddToWishButton_{0}_{1}\" name=\"AddToWishButton_{0}_{1}\" class=\"button add-to-wishlist\" src=\"{2}\" alt=\"{3}\" ", ProductID, VariantID, src, AppLogic.GetString("AppConfig.WishButtonPrompt", SkinID, LocaleSetting));
                ...
                {
                    // Render normal html button
                    tmpS.AppendFormat("<input type=\"button\" id=\"AddToWishButton_{0}_{1}\" name=\"AddToWishButton_{0}_{1}\" class=\"button add-to-wishlist\" value=\"" + AppLogic.GetString("AppConfig.WishButtonPrompt", SkinID, LocaleSetting) + "\" >", ProductID, VariantID);
                }
            }
        ...
        /// <returns></returns>
        static public String GetAddToCartForm(Customer ThisCustomer, bool forPack, bool forKit, bool showWishListButton, int ProductID, int VariantID, int SkinID, int DisplayFormat, String LocaleSetting, bool ColorChangeProductImage, VariantStyleEnum VariantStyle, bool insertItemsToKitCart, bool breakAttributes, string formPrefix)
        {
            int CartRecID = CommonLogic.QueryStringUSInt("CartRecID");
            String TextOptionForEdit = String.Empty;
            int QuantityForEdit = 0;
            ...
            if (!forPack)
            {
                ...
                if (isEditKit)
                {
                    ...
                    tmpS.Append(" <input id=\"" + FormName + "_AddToCartButton\" type=\"submit\" class=\"button call-to-action add-to-cart\" value=\"" + cartCaption + "\">");
                }
                else
                {
                    tmpS.Append(" <input id=\"" + FormName + "_AddToCartButton\" type=\"submit\" class=\"button call-to-action add-to-cart\" value=\"" + cartCaption + "\">");
            ...
            if (AppLogic.AppConfigBool("ShowWishButtons") && showWishListButton)
            {
                if (!isEditKit)
                {
					tmpS.Append(" <input id=\"" + FormName + "_AddToWishButton\" type=\"button\" class=\"button call-to-action add-to-wishlist\" value=\"" + AppLogic.GetString("AppConfig.WishButtonPrompt", SkinID, LocaleSetting) + "\" >");
                }
            }
			tmpS.Append("	</span>");
            tmpS.Append("</div>");
        ...
        public void ApplyShippingRules()
        {
            if (!AppLogic.AppConfigBool("SurpressCartShippingRulesCode"))
            {
                ...
                {
                    DB.ExecuteSQL("update shoppingcart set ShippingAddressID=" + ThisCustomer.PrimaryShippingAddressID.ToString() + " where CartType=" + ((int)CartTypeEnum.ShoppingCart).ToString() + " and CustomerID=" + ThisCustomer.CustomerID.ToString(), m_DBTrans);
                }
                LoadFromDB(CartType);
            }
        }
