    ...
    public class Tokens : ExpressionBuilder
    {
        ...
        public static object GetEvalData(string expression, Type target, string entry)
        {
            ...
            switch (command.ToLowerInvariant())
            {
				...
				case "escapedstringresource":
					return EscapedStringResourceParser(ThisCustomer, values);

				case "stringresourceformat":
                    return StringResourceFormatParser(ThisCustomer, values);

				case "formattedstringresource":
					return FormatStringResource(ThisCustomer, values);
				...
				case "globalconfigbool":
					return AppConfigBoolParser(values);

				case "globalconfigusint":
					return AppConfigUSIntParser(values);
		...
		private static int GlobalConfigUSIntParser(string[] values)
		{
			if (values.Length >= 2)
			{
				string name = values[1].Trim();
				return AppLogic.GlobalConfigUSInt(name);
			}
			else
			{
				throw new InvalidOperationException("GlobalConfig Name not specified!!!");
			}
		}
		...
		private static object GlobalConfigBoolParser(string[] values)
		{
			if (values.Length >= 2)
			{
				string name = values[1].Trim();
				return AppLogic.GlobalConfigBool(name);
			}
			else
			{
				throw new InvalidOperationException("GlobalConfig Name not specified!!!");
		...
		private static object GlobalConfigParser(string[] values)
		{
			if (values.Length >= 2)
			{
				string name = values[1].Trim();
				return AppLogic.GlobalConfig(name);
			}
			else
			{
				throw new InvalidOperationException("GlobalConfig Name not specified!!!");
		...
		private static string EscapedStringResourceParser(Customer thisCustomer, string[] values)
		{
			if(values.Length < 3)
				throw new InvalidOperationException("String resource key or escaping not specified");

			if(thisCustomer == null)
				throw new ArgumentNullException("thisCustomer");

			var resourceKey = values[1].Trim();
			var escaping = values[2].Trim();
			var resourceValue = AppLogic.GetString(resourceKey, thisCustomer.SkinID, thisCustomer.LocaleSetting);

			switch(escaping.ToLower())
			{ 
				case "javascript":
					return HttpUtility.JavaScriptStringEncode(resourceValue);

				case "html":
					return HttpUtility.HtmlEncode(resourceValue);

				case "html-attribute":
					return HttpUtility.HtmlAttributeEncode(resourceValue);

				case "url":
					return HttpUtility.UrlEncode(resourceValue);
		...
		private static string FormatStringResource(Customer thisCustomer, string[] values)
		{
			if(values.Length < 3)
				throw new InvalidOperationException("String resource key or format string not specified");

			if(thisCustomer == null)
				throw new ArgumentNullException("thisCustomer");

			var resourceKey = values[1].Trim();
			var formatString = values[2].Trim();
			var resourceValue = AppLogic.GetString(resourceKey, thisCustomer.SkinID, thisCustomer.LocaleSetting);

			return String.Format(formatString, resourceValue);
		}
