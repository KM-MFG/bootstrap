	...
	public class CheckOutPageController : ICheckOutPageController
	{
		...
		public virtual string GetStandardCheckoutPage()
		{
			...
			else
			{
				...
				if (this.ShoppingCart.HasMultipleShippingAddresses() && this.ShoppingCart.TotalQuantity() <= AppLogic.MultiShipMaxNumItemsAllowed() && this.ShoppingCart.CartAllowsShippingMethodSelection)
				{
		...
		{
			return !this.ShoppingCart.HasMultipleShippingAddresses() &&
		...
		public virtual bool CanUseInternationalCheckout()
		{
			...
			if (AppLogic.AppConfigBool("InternationalCheckout.Enabled"))
			{
				...
				if (!gAllUSAddresses && !this.ShoppingCart.HasDownloadComponents() && !this.ShoppingCart.HasCoupon()
