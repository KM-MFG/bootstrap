    ...
    public class Order
    {
        ...
        */
        ...
        public static void MarkOrderAsFraud(int OrderNumber, bool SetFraudStateOn)
        {
            ...
            {
                con.Open();
                using (IDataReader rs = DB.GetRS("Select CustomerID, CouponType, CouponCode from Orders  with (NOLOCK)  where OrderNumber=" + OrderNumber.ToString(), con))
                {
                    if (rs.Read())
                    {
                ...
                foreach (CartItem c in ord.CartItems)
                {
                    if (!AppLogic.ProductHasBeenDeleted(c.ProductID) && !AppLogic.VariantHasBeenDeleted(c.VariantID) && !c.IsRecurring)
                    {
                        ...
                        if (AppLogic.IsAKit(c.ProductID))
                        {
                            ...
                            cart.AddItem(ThisCustomer, ShipAddrID, c.ProductID, c.VariantID, c.Quantity, tmp, c.ChosenColorSKUModifier, c.ChosenSize, c.ChosenSizeSKUModifier, c.TextOption, CartTypeEnum.ShoppingCart, true, false, System.Decimal.Zero, composition);
                        ...
                        {
                            cart.AddItem(ThisCustomer, ShipAddrID, c.ProductID, c.VariantID, c.Quantity, c.ChosenColor, c.ChosenColorSKUModifier, c.ChosenSize, c.ChosenSizeSKUModifier, c.TextOption, CartTypeEnum.ShoppingCart, true, false, System.Decimal.Zero);
                        }
                    }
                }
            }
        ...
        public bool IsEditable()
        {
            ...
            if(ContainsGiftCard() || HasMultipleShippingAddresses() || HasRecurringComponents())
            {
                return false;
            }
